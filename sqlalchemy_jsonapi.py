...
Monkey-patched for Dradis usage. It is weird, wrong and old.
I shouldn't do that, I think. It's weird.
"""
...
class JSONAPI(object):
    ...
    def dump_relationship_data(self, item, obj, depth, fields, sort, include):
        ...
        for key, value in item.jsonapi_relationships_override.iteritems():
            if callable(value):
                relationships[key] = value(item)
            else:
    ...
    def serialize(self, to_serialize, depth=1, fields=None, sort=None,
        ...
        if is_single:
            ...
            to_return['meta']['count'] = to_serialize.limit(None).offset(None).count()
            to_return['meta']['offset'] = to_serialize._offset if to_serialize._offset is not None else 0
            to_return['meta']['limit'] = to_serialize._limit
